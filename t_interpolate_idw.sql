...
CREATE OR REPLACE FUNCTION  t_interpolate_idw(
...
BEGIN
	...
	*/
	...
	LOOP
		...
		RAISE NOTICE 'dv fx yx x y % % % %', finalPoint[x],finalPoint[y], distanceVector[x], distanceVector[y];
...
--		distance=(sqrt(   distanceVector[x]*distanceVector[x] + distanceVector[y]*distanceVector[y] + distanceVector[z]*distanceVector[z]        ) );
		distance=power(sqrt(   distanceVector[x]*distanceVector[x] + distanceVector[y]*distanceVector[y] ) ,idwpower);
